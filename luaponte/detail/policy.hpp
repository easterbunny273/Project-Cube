...
#ifndef LUAPONTE_DETAIL_POLICY_HPP
#define LUAPONTE_DETAIL_POLICY_HPP
...
#include <luaponte/detail/make_instance.hpp>

#include <boost/type_traits/add_reference.hpp>

#include <luaponte/detail/decorate_type.hpp>
...
// ********** pointer converter ***********
    ...
    struct pointer_converter
    {
        ...
        void apply(lua_State* L, T* ptr)
        {
            ...
            if (luaponte::get_back_reference(L, ptr))
                return;
...
// ******* value converter *******
    ...
    struct value_converter
    {
        ...
        {
            if (luaponte::get_back_reference(L, x))
                return;
...
// ******* reference converter *******
    ...
    struct ref_converter : pointer_converter
    {
        ...
        {
            if (luaponte::get_back_reference(L, ref))
                return;
    ...
    struct value_wrapper_converter
    {
        ...
        static int match(lua_State* L, by_const_reference<T>, int index)
        {
            return value_wrapper_traits<T>::check(L, index)
                ? (std::numeric_limits<int>::max)() / LUAPONTE_MAX_ARITY
                : -1;
        }
...
# define LUAPONTE_PLACEHOLDER_ARG(N) boost::arg<N>(*)()
...
#endif // LUAPONTE_DETAIL_POLICY_HPP
